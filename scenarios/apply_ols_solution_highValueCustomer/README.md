# OLS Model Application Scenario - High Value Customer Prediction
...
## Description
This docker-compose configuration applies the trained OLS Regression model to activation data for high-value customer prediction.
...
4. **executor_ols**: Executes the OLS prediction algorithm (python:3.9-slim)
...
### 5. View Logs
```bash
docker-compose logs -f executor_ols
```
...
## Expected Output
Predictions will be saved to the shared volume at `/tmp/predictions/ols_predictions.csv`.
